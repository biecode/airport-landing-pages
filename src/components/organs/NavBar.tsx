        ...
        >
          ...
          <div className="lg:flex hidden items-center gap-20">
              ...
              {NavLinks.map((navlink, index) => {
                return (
                  <List
                    ...
                    rest={undefined}
                    dropdown={
                      navlink.children ? ( // Check if children exist
                        <ul className="dropdown-menu">
                          {navlink.children.map((child, childIndex) => (
                            <li key={childIndex}>
                              <NavLink to={child.url} className="...">
                                {child.name}
                              </NavLink>
                            </li>
                          ))}
            ...
            <ul className="flex items-center justify-center gap-6">
              <List className="text-gray-950" rest={undefined}>
        ...
        >
          <section className="w-full px-4 py-6 flex flex-col gap-16">
            ...
            <ul className="flex flex-col gap-3 pl-2">
              {NavLinks.map((navlink, index) => (
                <List className="w-full text-base" key={index} rest={undefined}>
                  <NavLink
                    to={navlink.url}
                    onClick={handleToggle}
